...
bool traversetlb(int, int*);
void putintlb(int, int);
bool isemptytlb(int*);
...
int main()
{
	...
	tlb = (transbuf*)malloc(sizeof(transbuf)*64);
	memset(PA, 0, sizeof(int)*PA_SIZE);
	memset(table, 0, sizeof(int)*0x00A00000);
	memset(tlb, 0, sizeof(transbuf)*64);
	/*for(int i=0; i<100; i++){
		tlb[i].page_index = 0;
		tlb[i].page_fn = 0;
		tlb[i].sc = 0;
	...
	while (1)
	{
		...
		if(ret2 != -1){
			...
			for(int i=0; i<10; i++){
				...
				if(traversetlb(pageidx, &tlb_pointer)){//if it is TLB HIT
					...
					if((table[pageidx]&0xFFF) == 0x0){
						...
						putintlb(pageidx, table[pageidx]);
					}//not mapped
					else{
						putintlb(pageidx, table[pageidx]);
					}//mapped
				...
				real_addr = tlb[tlb_pointer].page_fn | offset;
...
void child_handler(int signo)
{
	...
	for(int i=0; i<5; i++)
	{
		raw_page.va[i] = VA; //rand() % 0x100000000;
...
bool traversetlb(int pagenum, int* i){

	for(*i=0; *i<64; *i++){
		if((tlb[*i].page_index == pagenum) && (tlb[*i].full == 1))
			return true;
	}
...
void putintlb(int pagenum,int framenum){
	int k;
	if(isemptytlb(&k)){//tlb has the empty space
		tlb[k].page_index = pagenum;
		tlb[k].page_fn = framenum;
		tlb[k].sc = 1;
		tlb[k].full = 1;
	}	
	else{//tlb is full
		for(int i=0; i<64; i++){
			if(tlb[i].sc == 1){
				...
				tlb[i].page_index = pagenum;
		                tlb[i].page_fn = framenum;
               			tlb[i].sc = 1;
                		tlb[i].full = 1;
			}
		}
